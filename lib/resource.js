...
class Resource {
    ...
    //获取类型名称
    static parseResourceName(tplname) {
    ...
    //获取源数据
    static getSource(sdopx, tplname, tplid) {
    ...
    getpath: function (tplname, sdopx) {
        ...
        if (tplname.substr(0, 1) == '@') {
            ...
            for (let key in tpldirs) {
                ...
                let fpath = path.join(tpldirs[key], tplname + '.' + sdopx_1.Sdopx.extension);
                if (fs.existsSync(fpath)) {
                    filepath = fpath;
                    break;
                }
