...
class Source {
    constructor(resource, sdopx, tplname, tplid, type, name) {
        //数据模板代码
        this.content = null;
        this.length = 0;
        this.isload = false;
    ...
    load() {
        ...
        if (content.length > 0 && sdopx_1.Sdopx.Filters['pre'] && sdopx_1.Sdopx.Filters['pre'] instanceof Array) {
            for (let i = 0; i < sdopx_1.Sdopx.Filters['pre'].length; i++) {
                let func = sdopx_1.Sdopx.Filters['pre'][i];
