...
const tree_map_1 = require("./tree_map");
...
class Lexer {
    ...
    analysis(tagname, rules) {
        ...
        let exp = syntaxrules_1.SyntaxRules.getRule(tagname);
        switch (mode) {
            case 0:
    ...
    match() {
        ...
        let token = syntaxrules_1.SyntaxRules.getToken(tag);
    ...
    lexTpl() {
        ...
        this.stack = []; //清空栈
        let tree = this.tree = new tree_map_1.TreeMap();
        if (sdopx_1.Sdopx.debug) {
            tree.setInfo(this.getSourceInfo());
        }
        let tag = 'Init';
        do {
            let next = syntaxrules_1.SyntaxRules.getNext(tag);
            this.initNext(next);
            let data = this.match();
            if (data === null) {
            ...
            //存栈
            let open = syntaxrules_1.SyntaxRules.getOpen(tag);
            if (open !== null) {
                //打开是时候清理
                if (open == 'tagattr') {
            ...
            //离栈
            let close = syntaxrules_1.SyntaxRules.getClose(tag);
            if (close !== null) {
                let endtag = this.stack.length == 0 ? null : this.stack[this.stack.length - 1];
