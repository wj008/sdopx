...
var Lexer = (function () {
    ...
    Lexer.prototype.analysis = function (tagname, rules) {
        ...
        var endtags = typeof (rules) == 'number' ? null : rules.flags;
        if (endtags) {
    ...
    Lexer.prototype.match = function () {
        ...
        var end = retinfo.end, start = retinfo.start, val = retinfo.val;
        switch (mode) {
            //找接受符号 不包括结束符号
            case 2:
                end = start;
    ...
    Lexer.prototype.lexTpl = function () {
        ...
        do {
            ...
            if (open_1 !== null) {
                //打开是时候清理
                if (open_1 == 'tagattr') {
                    var endtag = this.stack.length == 0 ? null : this.stack[this.stack.length - 1];
    ...
    Lexer.prototype.lexHtml = function () {
        ...
        if (source.end_literal) {
            ...
            if (ret2 && (!ret || ret2.start <= ret.start)) {
                ...
                return { map: '', code: code_1, next: 'Close_Literal' };
            }
        }
        //到尾部都没找到模板
        if (!ret) {
