...
var Parser = (function () {
    ...
    Parser.prototype.pars_tagname = function (item) {
        ...
        while (true) {
            ...
            if (item_1.tag == 'closeTagAttr') {
                continue;
            }
            if (item_1.tag == 'closeTpl') {
                temp.node = item_1.node;
                return temp;
            }
            if (item_1.tag == 'openTagAttr') {
    ...
    Parser.prototype.pars_block_open = function (item) {
        ...
        while (true) {
            ...
            if (item_2.map == 'closeTagAttr') {
                continue;
            }
            if (item_2.map == 'closeTpl') {
                temp['start'] = item_2.end;
                continue;
            }
            if (item_2.map == 'openTagAttr') {
    ...
    Parser.prototype.pars_modifier = function (item) {
        ...
        if (!(citem && citem.node == syntaxrules_1.Rules.BOUND_MODIFIER && citem.value.replace(/(^\s+|\s+$)/g, '') == ':')) {
            this.lexData.prev();
        }
        return temp;
    };
