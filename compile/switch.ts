...
//很可能不被加入
Compile.registerCompile('switch', (name, args, compile: Compile) => {
...
Compile.registerCompile('case', (name, args, compile: Compile) => {
...
Compile.registerCompile('default', (name, args, compile: Compile) => {
    let [tag,data]=compile.closeTag(['switch', 'case']);
    let output = [];
    if (tag == 'switch') {
        output.push(' */');
...
Compile.registerCompile('switch_close', (name, compile) => {
